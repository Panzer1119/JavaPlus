...
import de.codemakers.algorithms.astar.datastructures.ClosedSet;
import de.codemakers.algorithms.astar.datastructures.ClosedSetHash;
import de.codemakers.algorithms.astar.datastructures.IClosedSet;
import de.codemakers.algorithms.astar.datastructures.IOpenSet;
import de.codemakers.algorithms.astar.datastructures.OpenSet;
import java.util.ArrayList;

/**
 * Uses the A* Algorithm to find the shortest path from an initial to a goal
 * node.
 *
 */
public class AStar {
     ...
     * the A* heuristics (h must not overestimate). initialNode and last found
     * node included.
     */
    ...
    public ISearchNode search(ISearchNode initialNode, IGoalNode goalNode) {
        ...
        IClosedSet closedSet = implementsHash ? new ClosedSetHash(new SearchNodeComparator()) : new ClosedSet(new SearchNodeComparator());
    ...
    /**
     * returns path from the earliest ancestor to the node in the argument if
     * the parents are set via AStar search, it will return the path found. This
     * is the shortest shortest path, if the heuristic h does not overestimate
     * the true remaining costs
     *
     * @param node node from which the parents are to be found. Parents of the
     * node should have been properly set in preprocessing (f.e. AStar.search)
     * @return path to the node in the argument
     */
    public static List<ISearchNode> path(ISearchNode node) {
        List<ISearchNode> path = new ArrayList<>();
        path.add(node);
